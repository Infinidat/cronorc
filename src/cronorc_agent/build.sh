#!/bin/bash
#
# Based on: https://gist.githubusercontent.com/eduncan911/68775dba9d3c028181e4/raw/7b04444acb9d3a2236e075ae13df21184faee824/go-build-all
#
...
OUTPUT="../../cronorc_agent_dist"

for PLATFORM in $PLATFORMS; do
  GOOS=${PLATFORM%/*}
  GOARCH=${PLATFORM#*/}
  BIN_FILENAME="${OUTPUT}/${GOOS}/${GOARCH}/cronorc"
  if [[ "${GOOS}" == "windows" ]]; then BIN_FILENAME="${BIN_FILENAME}.exe"; fi
  CMD="GOOS=${GOOS} GOARCH=${GOARCH} ${COMPILER} build -o ${BIN_FILENAME} $@"
  echo "${CMD}"
...
if [[ $PLATFORMS_ARM == *"linux"* ]]; then
  CMD="GOOS=linux GOARCH=arm64 ${COMPILER} build -o ${OUTPUT}/linux/arm64/cronorc $@"
  echo "${CMD}"
  eval $CMD || FAILURES="${FAILURES} ${PLATFORM}"
fi
for GOOS in $PLATFORMS_ARM; do
  ...
  for GOARM in 7 6 5; do
    BIN_FILENAME="${OUTPUT}/${GOOS}/${GOARCH}${GOARM}/cronorc"
    CMD="GOARM=${GOARM} GOOS=${GOOS} GOARCH=${GOARCH} ${COMPILER} build -o ${BIN_FILENAME} $@"
    echo "${CMD}"
